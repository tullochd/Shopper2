...
/**
 * Created by tullochd on 1/28/2016.
 */
public class DBHandler extends SQLiteOpenHelper {
    ...
    public Cursor getShoppingLists(){
        ...
        SQLiteDatabase db = getWritableDatabase();
        //Select all the rows from data and rawQuery executes it.
    ...
    public  Cursor getShoppingListItems(Integer listId){
        ...
        return db.rawQuery("SELECT * FROM " + TABLE_SHOPPING_LIST_ITEM +
                " WHERE " + COLUMN_LIST_ID + " = " + listId, null);
    }
    ...
    public String getShoppingListTotalCost(int listId){
        ...
        int numShoppingListsItems = c.getCount();

        if (numShoppingListsItems >= 1){
            c.moveToFirst();
